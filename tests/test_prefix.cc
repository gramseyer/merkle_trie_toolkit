...
TEST_CASE("byte_prefix_match_len", "[prefix]")
{
    ...
    REQUIRE(key_buf.get_prefix_match_len(
                PrefixLenBits{ 64 }, key_buf2, PrefixLenBits{ 64 })
            == PrefixLenBits{ 36 });
}
...
TEST_CASE("uint64 write_node_header", "[prefix]")
{
    ...
    check({ 0x00, 0x3C, 0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x80 },
          PrefixLenBits{ 60 });
    check({ 0x00, 0x40, 0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89 },
          PrefixLenBits{ 64 });
}

TEST_CASE("bytearray write_node_header", "[prefix]")
{
    ByteArrayPrefix<16> key({ 0xAB,
                              0xCD,
                              0xEF,
                              0x01,
                              0x23,
    ...
    check({ 0x00, 0x40, 0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89 },
          PrefixLenBits{ 64 });
    check({ 0x00, 0x44, 0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89, 0xA0 },
          PrefixLenBits{ 68 });
}

TEST_CASE("bytearray test serialize", "[prefix]")
{
    ByteArrayPrefix<16> key({ 0xAB,
                              0xCD,
                              0xEF,
    ...
    auto bytes = key.get_bytes_array();

    ByteArrayPrefix<16> key2(bytes.data(), slice_ctor_t{});

    REQUIRE(key == key2);

    std::vector<uint8_t> partial;
    key.write_bytes_to(partial, PrefixLenBits{ 16 });
    std::vector<uint8_t> rest(14, 0);
    partial.insert(partial.end(), rest.begin(), rest.end());
    REQUIRE(partial.size() == 16);

    key2 = ByteArrayPrefix<16>(partial.data(), slice_ctor_t{});
    key.truncate(PrefixLenBits{ 16 });

    REQUIRE(key2 == key);
}

TEST_CASE("uint64 test serialize", "[prefix]")
{
    UInt64Prefix key(0xABCD'EF01'2345'6789);

    auto bytes = key.get_bytes_array();

    UInt64Prefix key2(bytes.data(), slice_ctor_t{});

    REQUIRE(key == key2);

    std::vector<uint8_t> partial;
    key.write_bytes_to(partial, PrefixLenBits{ 16 });
    std::vector<uint8_t> rest(6, 0);
    partial.insert(partial.end(), rest.begin(), rest.end());
    REQUIRE(partial.size() == 8);

    key2 = UInt64Prefix(partial.data(), slice_ctor_t{});
    key.truncate(PrefixLenBits{ 16 });
