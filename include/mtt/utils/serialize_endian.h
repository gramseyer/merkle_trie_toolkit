...
//! Reads contents of \a buf into \a value, starting from \a buf[offset], interpreted big endian
...
//! Reads contents of \a buf into \a value, starting from \a buf[offset], interpreted little endian
template<typename array, std::unsigned_integral T>
static void write_unsigned_little_endian(array& buf, const T& value, const size_t offset = 0) {
	constexpr size_t sz = sizeof(T);
	constexpr size_t buf_sz = sizeof(buf);
...
//! Serialize \a value in little-endian and write to \a buf (and subsequent bytes)
template<std::unsigned_integral T>
static void write_unsigned_little_endian(uint8_t* buf, const T& value) {
	constexpr size_t sz = sizeof(T);
...
template<std::unsigned_integral T>
static T read_unsigned_little_endian(const unsigned char* buf) {
	constexpr size_t sz = sizeof(T);
	T output = 0;
