...
class FixedChildrenMap
{
  ...
  public:
    ...
    uint16_t get_bv() const { return bv.get(); }
    ...
    underlying_ptr_t operator[](uint8_t idx) { return map[idx]; }
    ...
    */
    ...
    struct iterator_
    {
        ...
        using ptr_t = typename std::conditional<is_const,
                                                const underlying_ptr_t,
                                                underlying_ptr_t>::type;
    ...
    trie_ptr_t extract(uint8_t branch_bits)
    {
        ...
        if (!bv.contains(branch_bits)) {
            std::printf(
                "bad extraction of bb %u! bv was %x\n", branch_bits, bv.get());
    ...
    constexpr static iterator end() { return iterator{ 0, nullptr }; }
    ...
    bool empty() const { return bv.empty(); }
