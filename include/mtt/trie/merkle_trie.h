...
class TrieNode
{
  public:
    ...
    */
    using AtomicMetadataType =
        typename std::conditional<HAS_METADATA,
    ...
    static_assert(!std::is_void<MetadataType>::value,
                  "can't have void metadata");
  ...
  private:
    ...
    constexpr static unsigned char MAX_BRANCH_VALUE
        = ((_MAX_CHAR_VALUE)
           >> (8 - BRANCH_BITS)); // 0 to this value, inclusive
  public:
  ...
  private:
    ...
    void update_metadata(const MetadataType& metadata_delta)
    {
        if (HAS_METADATA) {
    ...
    PrefixLenBits get_prefix_match_len(const prefix_t& other,
    ...
    const MetadataType _insert(const prefix_t& key,
                               InsertedValueType&& leaf_value);

    Hash& get_hash_ptr() { return hash; }
    ...
    OptionalLock<USE_LOCKS>& get_lock_ref() { return locks; }
  ...
  public:
    ...
    TrieNode(children_map_t&& new_children,
    ...
    template<typename InsertFn, typename InsertedValueType>
    static trie_ptr_t make_value_leaf(
        const prefix_t& key,
            ...
            InsertedValueType&&>::type leaf_value)
    {
        ...
        return std::make_unique<TrieNode>(key, std::move(leaf_value), metadata);
    }
            ...
            InsertedValueType&&>::type leaf_value)
    {
        ...
        return std::make_unique<TrieNode>(key, std::move(value_out), metadata);
    }

    //! Constructor for value leaves.
    TrieNode(const prefix_t& key,
    ...
    void print_offsets()
    {
        TRIE_LOG("children   %lu %lu",
                 ...
                 sizeof(children));
        TRIE_LOG(
            "locks      %lu %lu", offsetof(TrieNode, locks), sizeof(locks));
        TRIE_LOG(
            "prefix     %lu %lu", offsetof(TrieNode, prefix), sizeof(prefix));
        TRIE_LOG("prefix_len %lu %lu",
                 offsetof(TrieNode, prefix_len),
                 sizeof(prefix_len));
        TRIE_LOG("metadata   %lu %lu",
                 offsetof(TrieNode, metadata),
                 sizeof(metadata));
        TRIE_LOG("hash       %lu %lu", offsetof(TrieNode, hash), sizeof(hash));
        TRIE_LOG("hash_valid %lu %lu",
                 offsetof(TrieNode, hash_valid),
                 sizeof(hash_valid));
    }
    ...
    struct iterator
    {
        ...
        using kv_t = std::pair<const prefix_t,
                               std::reference_wrapper<const ValueType>>;
        ...
        {
            if (main_node.prefix_len == MAX_KEY_LEN_BITS) {
                return std::make_pair(main_node.prefix,
                                      std::cref(main_node.children.value()));
            }
        ...
        bool at_end() { return local_iter == main_node.children.end(); }
    };

    const PrefixLenBits get_prefix_len() const { return prefix_len; }

    const prefix_t get_prefix() const { return prefix; }
    ...
    //! empty trie.
    static trie_ptr_t make_empty_node() { return std::make_unique<TrieNode>(); }
    ...
    ValueType* get_value(const prefix_t& query_key);

    //! Get the value associated with a given key.
    //! throws if key is not present

    const ValueType* get_value_nolocks(const prefix_t& query_key) const;
    ...
    typename std::enable_if<x, size_t>::type num_deleted_subnodes() const
    {
        ...
        int64_t out
            = metadata.num_deleted_subnodes.load(std::memory_order_acquire);
        if (out < 0) {
    ...
    const MetadataType get_metadata_unsafe() { return metadata.unsafe_load(); }

    void set_metadata_unsafe(const AtomicMetadataType& other)
    {
    ...
    std::vector<TrieNode*> children_list_ordered() const
    {
        ...
        for (unsigned int branch_bits = 0; branch_bits <= MAX_BRANCH_VALUE;
             branch_bits++) {
    ...
    */
    std::tuple<bool, MetadataType, trie_ptr_t> destructive_steal_child(
    ...
    //! modify operations.
    void invalidate_hash_to_node_nolocks(TrieNode* target);

    void invalidate_hash_to_key_nolocks(const prefix_t& query_key);
    ...
    {
        {
            [[maybe_unused]] auto lock
                = locks.template lock<TrieNode::exclusive_lock_t>();
            if (size() == 0) {
    ...
    template<typename MetadataOutputType,
             typename KeyInterpretationType,
             typename KeyMakerF>
    void metadata_traversal(std::vector<IndexedMetadata<MetadataOutputType,
    ...
    template<bool x = METADATA_DELETABLE>
    typename std::
        enable_if<x, std::tuple<MetadataType, std::optional<ValueType>>>::type
        mark_for_deletion(const prefix_t key);

    std::optional<MetadataType> mark_for_deletion_nolocks(const prefix_t& key);
    ...
    template<bool x = METADATA_DELETABLE>
    typename std::enable_if<x, void>::type clear_marked_deletions();

    //! Perform deletion immediately.
    //! msg to parent: delete child, is anything deleted?, metadata change
    std::tuple<bool, std::optional<ValueType>, MetadataType> perform_deletion(
        const prefix_t key);
    ...
    bool contains_key(const prefix_t& key) const;

    bool single_child() const { return children.size() == 1; }
    ...
    trie_ptr_t endow_split(int64_t endow_threshold);

    int64_t endow_lt_key(const prefix_t max_key) const;
    ...
    std::optional<prefix_t> get_lowest_key() const;
    ...
    void modify_value_nolocks(const prefix_t& query_prefix, ModifyFn& fn);

    template<typename ModifyFn>
    void modify_value_nolocks(const prefix_t& query_prefix);

    // concurrent modification will cause problems here
    TrieNode* get_subnode_ref_nolocks(const prefix_t& query_prefix,
                                      const PrefixLenBits query_len);

    template<typename VectorType>
    void accumulate_values_parallel_worker(VectorType& vec,
                                           size_t offset) const;
    ...
    */
    ...
    typename std::enable_if<x, void>::type clear_rollback();
...
class MerkleTrie
{
  ...
  public:
    ...
    template<typename... ApplyFn>
    void hash(Hash& buf, std::optional<HashLog<prefix_t>>& = null_log);
    ...
    void print_offsets() { root->print_offsets(); }
    ...
    struct iterator
    {
        ...
        bool at_end() { return iter.at_end(); }

        iterator(MerkleTrie& trie)
            : iter(*trie.root)
        {}
    };
    ...
    bool metadata_integrity_check() { return root->metadata_integrity_check(); }
    ...
    MetadataType metadata_query(const prefix_t query_prefix,
        ...
        typename std::enable_if<x, const uint16_t>::type query_len_bits)
    {
        ...
        using index_t = IndexedMetadata<MetadataOutputType,
                                        KeyInterpretationType,
                                        KeyMakerF>;
        ...
        vec.push_back(
            index_t(KeyMakerF::eval(zero_prefix), acc)); // start with 0 at bot
    ...
    template<typename ApplyFn>
    void parallel_apply(ApplyFn& fn) const;
    ...
    ValueType* get_value(const prefix_t& query_key)
    {
        ...
        return const_cast<ValueType*>(
            const_cast<const MerkleTrie*>(this)->get_value(query_key));
    }

    const ValueType* get_value(const prefix_t& query_key) const
    {

        static_assert(HAS_VALUE, "must have values to get a value");
    ...
    ValueType* get_value_nolocks(const prefix_t& query_key)
    {
        return const_cast<ValueType*>(
            const_cast<const MerkleTrie*>(this)->get_value_nolocks(query_key));
    }
    ...
    TrieT* get_subnode_ref_nolocks(const prefix_t query_prefix,
                                   const PrefixLenBits query_len_bits)
    {
    ...
    TrieT* __attribute__((warn_unused_result))
    ...
    void insert(const prefix_t& data, InsertedValueType&& leaf_value)
    {
        static_assert(HAS_VALUE, "cannot insert value in no-value-mode");
        std::lock_guard lock(*hash_modify_mtx);

        invalidate_hash();
        root->template insert<HAS_VALUE, InsertFn, InsertedValueType>(
            data, std::move(leaf_value));
    }

    template<typename InsertFn = OverwriteInsertFn<ValueType>>
    void insert(const prefix_t& data)
    {
        std::lock_guard lock(*hash_modify_mtx);
    ...
    bool mark_for_deletion_nolocks(const prefix_t& key)
    {
        static_assert(METADATA_DELETABLE,
                      "cannot delete without deletion metadata");
    ...
    template<bool x = METADATA_DELETABLE>
    typename std::enable_if<x, void>::type perform_marked_deletions()
    {
        ...
        DelSideEffectFn& side_effect_handler)
    {
        static_assert(x == METADATA_DELETABLE, "no funny business");
        std::lock_guard lock(*hash_modify_mtx);
        if (root->get_metadata_unsafe().num_deleted_subnodes
            == 0) { // ok bc exclusive lock on root
            return;
        }
    ...
    */
    ...
    void do_rollback()
    {
        ...
        static_assert(METADATA_ROLLBACK,
                      "need rollback metadata to call rollback");
...
TEMPLATE_SIGNATURE
const MetadataType
TrieNode<TEMPLATE_PARAMS>::metadata_query(const prefix_t query_prefix,
                                          const PrefixLenBits query_len)
{
...
template<typename MetadataOutputType,
         ...
         typename KeyMakerF>
void
TrieNode<TEMPLATE_PARAMS>::metadata_traversal(
    std::vector<
        IndexedMetadata<MetadataOutputType, KeyInterpretationType, KeyMakerF>>&
        vec,
    MetadataOutputType& acc_metadata,
    const PrefixLenBits query_len)
{
    ...
    if (prefix_len >= query_len) {
        ...
        acc_metadata
            += MetadataOutputType(interpreted_key, metadata.unsafe_load());

        vec.push_back(index_t(interpreted_key, acc_metadata));
        return;
    }

    for (uint8_t branch_bits = 0; branch_bits <= MAX_BRANCH_VALUE;
         branch_bits++) {
        auto iter = children.find(branch_bits);
        if (iter != children.end()) {

            (*iter).second->metadata_traversal(vec, acc_metadata, query_len);
        }
    }
}
...
TEMPLATE_SIGNATURE
void
MerkleTrie<TEMPLATE_PARAMS>::get_root_hash(Hash& out, std::optional<HashLog<prefix_t>>& log)
{
...
template<typename... ApplyFn>
void
MerkleTrie<TEMPLATE_PARAMS>::serial_hash(Hash& buffer)
{
...
template<typename... ApplyFn>
void
MerkleTrie<TEMPLATE_PARAMS>::hash(Hash& buffer, std::optional<HashLog<prefix_t>>& log)
{
    std::lock_guard lock(*hash_modify_mtx);
    ...
    const PrefixLenBits stealing_prefix_len)
{
    ...
    auto prefix_match_len
        = get_prefix_match_len(stealing_prefix, stealing_prefix_len);

    if (prefix_match_len
        == stealing_prefix_len) { // prefix_match_len <= prefix_len
    ...
    if (prefix_match_len == prefix_len) {
        ...
        auto [do_steal_entire_subtree, meta_delta, ptr]
            = (*iter).second->destructive_steal_child(stealing_prefix,
                                                      stealing_prefix_len);
...
TEMPLATE_SIGNATURE
void
TrieNode<TEMPLATE_PARAMS>::propagate_metadata(TrieNode* target,
...
TEMPLATE_SIGNATURE
void
TrieNode<TEMPLATE_PARAMS>::invalidate_hash_to_key_nolocks(
    const prefix_t& query_key)
{
    ...
    auto iter = children.find(branch_bits);
    if (iter == children.end()) {
        throw std::runtime_error("can't invalidate hash to nonexistent node");
    }
...
TrieNode<TEMPLATE_PARAMS>::create_proof_node()
{
    if (prefix_len == MAX_KEY_LEN_BITS) {
        ProofNode output;
        utils::write_unsigned_big_endian(output.prefix_length_and_bv.data(),
                                         prefix_len.len);
        return output;
    }
...
TEMPLATE_SIGNATURE
void
TrieNode<TEMPLATE_PARAMS>::create_proof(Proof& proof, prefix_t data)
{
...
void
TrieNode<TEMPLATE_PARAMS>::insert(
...
template<typename InsertFn, typename InsertedValueType>
const MetadataType
TrieNode<TEMPLATE_PARAMS>::_insert(const prefix_t& data,
                                   InsertedValueType&& leaf_value)
{
    ...
    if (prefix_len.len == 0 && children.empty()) {
        ...
        if (iter != children.end()) {
            ...
            if (HAS_METADATA) {
                auto metadata_delta = child->template _insert<InsertFn>(
                    data, std::move(leaf_value));
        ...
        auto new_child_metadata
            = new_child->metadata
                  .unsafe_load(); // ok bc exclusive lock on *this
...
TrieNode<TEMPLATE_PARAMS>::_merge_in(trie_ptr_t&& other)
{
    ...
    if (children.size() == 0 && prefix_len.len == 0) {
        throw std::runtime_error("cannot _merge into empty trie!  Must call "
                                 "merge or somehow guard against this.");
    }
    ...
    if (prefix_match_len == MAX_KEY_LEN_BITS) { // merge case 0
        ...
        if (HAS_METADATA) {
            ...
            auto metadata_delta
                = MergeFn::metadata_merge(metadata, other->metadata);
    ...
    if (prefix_len == other->prefix_len && prefix_len == prefix_match_len) {
        ...
        for (auto other_iter = other->children.begin();
            ...
            if (main_iter == children.end()) {
                ...
                children.emplace((*other_iter).first,
    ...
    if (other->prefix_len
        == prefix_match_len /* and thus prefix_len > prefix_match_len*/) {
        ...
        if (iter == children.end()) {
            ...
            auto original_child_metadata
                = original_child_branch->metadata
                      .unsafe_load(); // safe bc obj is threadlocal rn
            children.emplace(original_child_branch_bits,
                             std::move(original_child_branch));
            if (HAS_METADATA) {
                update_metadata(original_child_metadata);
                return other->metadata
                    .unsafe_load(); // safe bc move gives exclusive access to
                                    // thread
            } else {
                return MetadataType{};
            }
            ...
            auto original_metadata
                = original_child_branch->metadata
                      .unsafe_load(); // safe bc original_child_branch is
                                      // threadlocal
            ...
            if (!matching_subtree_of_other) {
                throw std::runtime_error(
                    "matching_subtree_of_other is null???");
            }

            // We do the swap here so that the input to _merge is always
            // destructible, as per invariant.
            children.emplace(original_child_branch_bits,
                             std::move(original_child_branch));

            // metadata adjustment corresponding to swapping pre-merge the
            // matching subtrees
            ...
            // auto meta_delta =
            // iter->second->_merge_in(std::move(original_child_branch));
            // TRIE_INFO_F(_log("post case 3 recursion:    "));
            if (HAS_METADATA) {
                update_metadata(meta_delta);
                // metadata += meta_delta;
                auto change_from_original
                    = metadata.unsafe_load(); // safe bc exclusive lock on this
    ...
    auto original_child_branch = move_contents_to_new_node();
    if (HAS_METADATA) {
        original_child_branch->set_metadata_unsafe(
            metadata); // safe bc obj is threadlocal and exclusive lock on
                       // metadata
    }
    children.clear();
    prefix_len = prefix_match_len;
    ...
    auto other_metadata
        = other->metadata.unsafe_load(); // safe bc other is xvalue etc
    ...
    if (HAS_METADATA) {
        ...
        auto original_child_metadata
            = metadata.unsafe_load(); // safe bc exclusve lock on self
        metadata.clear();
...
TEMPLATE_SIGNATURE
PrefixLenBits
TrieNode<TEMPLATE_PARAMS>::get_prefix_match_len(
...
TEMPLATE_SIGNATURE
template<bool x>
typename std::enable_if<x, std::tuple<MetadataType, std::optional<ValueType>>>::
    type
    ...
    DelSideEffectFn& side_effect_handler)
{
    ...
    if (prefix_len == MAX_KEY_LEN_BITS && metadata.num_deleted_subnodes == 1) {
        ...
        return std::make_pair(
            true, -metadata.unsafe_load()); // safe bc exclusive lock
    }
...
{
    [[maybe_unused]] auto lock = locks.template lock<exclusive_lock_t>();
    invalidate_hash();
    ...
    for (uint8_t bb = 0; bb <= MAX_BRANCH_VALUE;) {
        ...
        if (iter == children.end()) {
            ...
            if (ptr->single_child()) {
                children.emplace((*iter).first,
                                 std::move(ptr->get_single_child()));
            } else {
                bb++;
            }
        }
...
TrieNode<TEMPLATE_PARAMS>::clear_marked_deletions()
{
    ...
    if (metadata.unsafe_load().num_deleted_subnodes
        == 0) { // ok bc root gets exclusive lock
        return;
    }
...
TrieNode<TEMPLATE_PARAMS>::perform_deletion(const prefix_t key)
{
    ...
    TRIE_INFO("deleting key %s", key.to_string(MAX_KEY_LEN_BITS).c_str());
...
TrieNode<TEMPLATE_PARAMS>::endow_split(int64_t endow_threshold)
{
    ...
    for (unsigned char branch_bits = 0; branch_bits <= MAX_BRANCH_VALUE;
         branch_bits++) {
        ...
        if (iter != children.end()) {
            ...
            if (fully_consumed_subtree <= endow_threshold) {
                ...
                auto split_obj
                    = (*iter).second->endow_split(endow_threshold - acc_endow);
                if (split_obj) {
                    update_metadata(-(split_obj->metadata.unsafe_load()));
                    new_node_children.emplace((*iter).first,
                                              std::move(split_obj));
                }
    ...
    if (new_node_children.size()) {
        ...
        auto output
            = duplicate_node_with_new_children(std::move(new_node_children));
    ...
    VectorType& output) const
{
    ...
    tbb::parallel_for(range, [&output](const auto& range) {
        auto vector_offset = range.vector_offset;
        for (size_t i = 0; i < range.work_list.size(); i++) {
            range.work_list[i]->accumulate_values_parallel_worker(
...
template<typename VectorType>
void
TrieNode<TEMPLATE_PARAMS>::accumulate_values_parallel_worker(
    VectorType& vec,
    size_t offset) const
{
    if (prefix_len == MAX_KEY_LEN_BITS) {
    ...
    for (unsigned int branch_bits = 0; branch_bits <= MAX_BRANCH_VALUE;
        ...
        if (iter != children.end()) {
            if (!(*iter).second) {
                _log("bad node");
                throw std::runtime_error(
                    "tried to accumulate value from empty node");
            }
...
MerkleTrie<TEMPLATE_PARAMS>::parallel_apply(ApplyFn& fn) const
{
    ...
    tbb::parallel_for(range, [&fn](const auto& r) {
    ...
    ValueModifyFn& fn) const
{
    ...
    tbb::parallel_for(range, [&fn](const auto& range) {
    ...
    const PrefixLenBits query_len)
{
    ...
    if (prefix_match_len == prefix_len) {
        ...
        if (!(*iter).second) {
            _log("bad node: ");
            throw std::runtime_error(
                "found a null iter in get_subnode_ref_nolocks");
        }

        auto child_candidate
            = (*iter).second->get_subnode_ref_nolocks(query_prefix, query_len);
...
template<typename ModifyFn>
void
TrieNode<TEMPLATE_PARAMS>::modify_value_nolocks(const prefix_t& query_prefix,
                                                ModifyFn& fn)
{
...
TrieNode<TEMPLATE_PARAMS>::apply(ApplyFn& func)
{
    ...
    for (auto iter = children.begin(); iter != children.end(); iter++) {
        if (!(*iter).second) {
            throw std::runtime_error("null pointer len = "
                                     + std::to_string(prefix_len.len));
        }
        if ((*iter).first > MAX_BRANCH_VALUE) {
...
template<typename ApplyFn>
void
TrieNode<TEMPLATE_PARAMS>::apply_geq_key(ApplyFn& func,
                                         const prefix_t min_apply_key)
{
    ...
    if (prefix < min_apply_key_truncated) {
        ...
        // DebugUtils::__array_to_str(prefix,
...
template<typename ApplyFn>
void
TrieNode<TEMPLATE_PARAMS>::apply_lt_key(ApplyFn& func,
                                        const prefix_t threshold_key)
{
...
TrieNode<TEMPLATE_PARAMS>::get_lowest_key() const
{
    [[maybe_unused]] auto lock = locks.template lock<shared_lock_t>();
...
TEMPLATE_SIGNATURE
ValueType*
TrieNode<TEMPLATE_PARAMS>::get_value(const prefix_t& query_key)
{
    [[maybe_unused]] auto lock = locks.template lock<TrieNode::shared_lock_t>();
...
TrieNode<TEMPLATE_PARAMS>::clear_rollback()
{
    ...
    if (metadata.unsafe_load().num_rollback_subnodes
        == 0) { // ok bc root gets exclusive lock
        return;
    }
    ...
    for (unsigned int branch_bits = 0; branch_bits <= MAX_BRANCH_VALUE;
        ...
        if (result.first) {
            ...
            if (child_ptr->single_child()) {
                ...
                auto replacement_child_ptr = child_ptr->get_single_child();
                children.emplace((*iter).first,
                                 std::move(replacement_child_ptr));
            }
        }
        metadata_delta += result.second;
    }
